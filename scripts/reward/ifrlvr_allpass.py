"""IF-RLVR reward with an all-constraints-passed objective.

This keeps a small dense partial-credit component while making complete
constraint satisfaction the dominant optimization signal:

    reward = 0.2 * (passed / total) + 0.8 * int(passed == total)

The original partial-only reward remains in ``ifrlvr.py`` for reproducibility.
"""
...
def _load_constraint(ground_truth: str | dict[str, Any]) -> dict[str, Any]:
    """Normalize the serialized IF-RLVR constraint format."""
...
def reward_func(data_source: str, solution_str: str, ground_truth: str, extra_info=None, **_kwargs) -> dict[str, Any]:
    """Score a completion with partial credit plus a dominant all-pass bonus."""
    ...
    try:
        ...
        partial = passed / max(total, 1)
        all_pass = float(passed == total)
        score = PARTIAL_WEIGHT * partial + ALL_PASS_WEIGHT * all_pass
        return {
            "score": float(score),
            "constraint_pass_rate": float(partial),
            "all_constraints_passed": all_pass,
