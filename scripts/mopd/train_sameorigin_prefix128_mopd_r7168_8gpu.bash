#!/usr/bin/env bash
# Same-origin Prefix-128 MOPD: General-SFT ckp1 student and three same-origin RL teachers.
set -euo pipefail
...
STUDENT_GPUS=5
EXPECTED_STEP=484
RUN_NAME=${RUN_NAME:-q8b_sameorigin_prefix128_mopd_r7168_n1_b140_mb40_ep1_lr1e-6}
PREFIX_SFT_COEF=${PREFIX_SFT_COEF:-0.1}
...
PREFIX_DATASET=datasets/mopd/q8b_sameorigin_mopd_math_code_instruct_teacher_prefix128_think.parquet
...
test -f "${PREFIX_DATASET}" || { echo "Missing Prefix-128 dataset: ${PREFIX_DATASET}" >&2; exit 1; }
ROWS=$("${PYTHON_BIN}" -c "import pyarrow.parquet as pq; print(pq.ParquetFile('${PREFIX_DATASET}').metadata.num_rows)")
test "${ROWS}" -eq 67836 || { echo "Unexpected Prefix-128 rows: ${ROWS}" >&2; exit 1; }

if [[ "${MOPD_CONFIG_ONLY:-0}" != "1" ]] && [[ -f "${MERGED_MODEL}/config.json" ]] && compgen -G "${MERGED_MODEL}/*.safetensors" >/dev/null; then
  echo "Same-origin Prefix-128 MOPD already merged: ${MERGED_MODEL}"
  exit 0
fi
...
if [[ "${MOPD_CONFIG_ONLY:-0}" == "1" || ! -d "${ACTOR_CHECKPOINT}" ]]; then
  "${PYTHON_BIN}" -m verl.trainer.main_ppo \
    ...
    data.train_files="${PREFIX_DATASET}" data.val_files="${PREFIX_DATASET}" \
    ...
    data.max_prompt_length=2176 +data.teacher_prefix_max_len=128 data.max_response_length=7168 \
    ...
    +actor_rollout_ref.actor.teacher_prefix_sft_loss_coef="${PREFIX_SFT_COEF}" actor_rollout_ref.actor.teacher_prefix_sft_loss_agg_mode=token-mean \
    actor_rollout_ref.actor.ppo_mini_batch_size=40 actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=1 \
    actor_rollout_ref.actor.use_dynamic_bsz=True actor_rollout_ref.actor.ppo_max_token_len_per_gpu=9344 \
    ...
    actor_rollout_ref.rollout.gpu_memory_utilization=0.70 actor_rollout_ref.rollout.max_model_len=9344 \
...
echo "Same-origin Prefix-128 MOPD complete: ${MERGED_MODEL}"
