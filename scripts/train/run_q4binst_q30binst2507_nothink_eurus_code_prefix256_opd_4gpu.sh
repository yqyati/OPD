#!/usr/bin/env bash
# Completion-aware code prefix OPD:
...
export PREFIX_TRAIN_DATASET="${PREFIX_TRAIN_DATASET:-datasets/teacher_prefix/q4binst_q30binst2507_nothink_eurus_code_prefix256.parquet}"
export PREFIX_TRAIN_DATASET_NAME="Eurus-RL-Code-4BInst-30BInst-NoThink-Prefix256-CompletionAware"
export TEACHER_PREFIX_SFT_LOSS_COEF="${TEACHER_PREFIX_SFT_LOSS_COEF:-0.1}"
SFT_COEF_TAG="${TEACHER_PREFIX_SFT_LOSS_COEF/./p}"
export PREFIX_MODEL_OUTPUT_NAME_PREFIX="q4binst_q30binst2507_nothink_eurus_code_prefix256_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5"
...
export DATA_SEED=42
# The plain code run accepted native prompts up to 2048 tokens.  Allocate the
# additional 256 teacher-prefix tokens explicitly rather than silently
# filtering otherwise valid code prompts at the handoff boundary.
...
# Pure token-OPD: use only the teacher-derived rm_scores already attached to
# each rollout.  Do not decode/execute code or launch CPU code verifiers.
export DISABLE_CUSTOM_REWARD_FUNCTION=True
# ``batch`` returns rm_scores immediately when they are present, unlike the
# default ``naive`` manager which still computes a true reward for logging.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-q4binst_q30binst2507_nothink_eurus_code_prefix256_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5_${RUN_TAG}}"
...
# This is the registered LiveCodeBench style; --local_model_path supplies the
# actual experiment checkpoint.
