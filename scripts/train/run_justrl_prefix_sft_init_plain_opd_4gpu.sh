...
export MAX_VAL_RESP_LENGTH=7168 # Trainer validation is disabled; final eval below uses 15000.
...
if [ -z "${TEACHER_PREFIX_SFT_LOSS_COEF+x}" ]; then
    if [ "$RUN_MODE" = "prefix" ]; then
        export TEACHER_PREFIX_SFT_LOSS_COEF=0.1
    else
...
if [ "$RUN_MODE" = "plain" ]; then
    ...
    export TRAIN_DATASET_NAME=JustRL-PrefixSFTInit-PlainOPD-n1
    export MODEL_OUTPUT_NAME_PREFIX=opd_justrl_prefix_sft_init_plain_opd_n${N_RESPONSES}_lr${LR}
...
export ACTOR_MODEL_PATH=/mnt/shared-storage-gpfs2/p1-shared-2/yangqingyu/OPD/merged_models/justrl_teacher_prefix128_pure_sft_lr1e-5_step279
...
# export ACTOR_MODEL_PATH=model/DS-1.5B-SFT
export ACTOR_MODEL_NAME=justrl_prefix_sft_init
...
max_len = 0
prefixes = df["teacher_prefix_text"] if "teacher_prefix_text" in df.columns else [""] * len(df)
for prompt, prefix in zip(df["prompt"], prefixes):
...
echo "MAX_FULL_PROMPT_LEN: ${MAX_FULL_PROMPT_LEN}"
if [ "${MAX_FULL_PROMPT_LEN}" -gt "${MAX_PROMPT_LENGTH}" ]; then
