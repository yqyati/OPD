#!/usr/bin/env bash
# Completion-aware fixed-prefix-32 code OPD:
...
export PREFIX_TRAIN_DATASET="${PREFIX_TRAIN_DATASET:-datasets/teacher_prefix/q4binst_q30binst2507_nothink_eurus_code_prefix32.parquet}"
export PREFIX_TRAIN_DATASET_NAME="Eurus-RL-Code-4BInst-30BInst-NoThink-Prefix32-CompletionAware"
export TEACHER_PREFIX_SFT_LOSS_COEF="${TEACHER_PREFIX_SFT_LOSS_COEF:-0.1}"
SFT_COEF_TAG="${TEACHER_PREFIX_SFT_LOSS_COEF/./p}"
export PREFIX_MODEL_OUTPUT_NAME_PREFIX="${PREFIX_MODEL_OUTPUT_NAME_PREFIX:-q4binst_q30binst2507_nothink_eurus_code_prefix32_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5}"
...
export TEACHER_PREFIX_MAX_LEN=32
export TEACHER_PREFIX_SOFT_KL_LOSS_COEF=0.0

export ENABLE_THINKING=False
export DATA_SHUFFLE=True
export DATA_SEED=42
# Preserve the 2048-token native code prompt budget and reserve the fixed
# 32-token teacher prefix at the handoff boundary.
export MAX_PROMPT_LENGTH=2080
...
# Pure token-OPD: use teacher-derived rm_scores already attached to each
# rollout. ``batch`` returns those scores without decoding or executing code,
# so no CPU code verifier is launched during training.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-q4binst_q30binst2507_nothink_eurus_code_prefix32_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5_${RUN_TAG}}"
