#!/usr/bin/env bash
# Plain OPD started from the exact prefix512 pure-SFT warm start.
set -euo pipefail

source .env
cd "${OPD_ROOT}"

export SFT_MODEL_DIR="${SFT_MODEL_DIR:-${OPD_ROOT}/merged_models/q4binst_q30binst2507_nothink_prefix512_pure_sft_b96_lr1e-5_step186}"
test -f "${SFT_MODEL_DIR}/config.json" || { echo "Missing merged SFT model: ${SFT_MODEL_DIR}" >&2; exit 1; }

# Keep every plain-OPD choice identical to the completed Instruct<-Instruct
# baseline, apart from loading the SFT warm-start model.
export RUN_MODE=plain
export PLAIN_TRAIN_DATASET="datasets/dapo-math-17k-teacher-aligned.parquet"
export PLAIN_TRAIN_DATASET_NAME="q4binst_q30binst2507_nothink_plain_opd_from_prefix512_sft"
export PLAIN_MODEL_OUTPUT_NAME_PREFIX="q4binst_q30binst2507_nothink_prefix512sftinit_plain_opd_r4096_b96_n1_lr1e-5"
...
export EVAL_OUTPUT_DIR="${OPD_ROOT}/outputs/eval/q4binst_q30binst2507_nothink_prefix512sftinit_plain"

RUN_TAG="${RUN_TAG:-$(date +%Y-%m-%d_%H-%M-%S)}"
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-q4binst_q30binst2507_nothink_prefix512sftinit_plain_opd_r4096_b96_n1_lr1e-5_${RUN_TAG}}"
