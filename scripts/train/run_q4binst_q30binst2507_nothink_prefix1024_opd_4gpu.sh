#!/usr/bin/env bash
# Completion-aware fixed-prefix-1024 OPD:
# Qwen3-4B-Instruct-2507 <- Qwen3-30B-A3B-Instruct-2507, native no-think.
# Local train + merge + final math evaluation only; never submits an rjob.
set -euo pipefail

source .env
cd "${OPD_ROOT}"

# Controlled fixed-prefix counterpart to prefix128/256/512/768 and to the
# completed 1024->0 curriculum.  Keep the established fixed-prefix protocol.
...
export PREFIX_TRAIN_DATASET_NAME="q4binst_q30binst2507_nothink_prefix1024_completionaware"
export TEACHER_PREFIX_SFT_LOSS_COEF="${TEACHER_PREFIX_SFT_LOSS_COEF:-0.1}"
SFT_COEF_TAG="${TEACHER_PREFIX_SFT_LOSS_COEF/./p}"
export PREFIX_MODEL_OUTPUT_NAME_PREFIX="q4binst_q30binst2507_nothink_prefix1024_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5"
...
export DATA_SEED=42
# Match the completed fixed-prefix 128/256/512/768 controls exactly.
...
export EVAL_OUTPUT_DIR="${OPD_ROOT}/outputs/eval/q4binst_q30binst2507_nothink_prefix1024"

RUN_TAG="${RUN_TAG:-$(date +%Y-%m-%d_%H-%M-%S)}"
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-q4binst_q30binst2507_nothink_prefix1024_sft${SFT_COEF_TAG}_opd_r4096_b96_n1_lr1e-5_${RUN_TAG}}"
export CKPT_PATH="${CKPT_PATH:-checkpoint/${EXPERIMENT_NAME}}"

test -f "${PREFIX_TRAIN_DATASET}" || {
    echo "Missing fixed-prefix1024 dataset: ${PREFIX_TRAIN_DATASET}" >&2
    exit 1
}
