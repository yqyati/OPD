#!/usr/bin/env bash
# Plain OPD initialized from the full-4096 pure-SFT control checkpoint:
# Qwen3-1.7B-Base <- Qwen3-4B-Base-GRPO.  No prefix is supplied in this stage.
set -euo pipefail
...
test -f "${SFT_MODEL_DIR}/config.json" || {
    echo "Missing merged full-response SFT checkpoint: ${SFT_MODEL_DIR}" >&2
    exit 1
}

export RUN_MODE=plain
export PLAIN_TRAIN_DATASET="datasets/dapo-math-17k-teacher-aligned.parquet"
export PLAIN_TRAIN_DATASET_NAME="q3b_q4bgrpo_nothink_plain_opd_from_full4096_sft"
export PLAIN_MODEL_OUTPUT_NAME_PREFIX="q3b_q4bgrpo_nothink_full4096sftinit_plain_opd_r4096_b64_n1_lr1e-5"
...
export EVAL_OUTPUT_DIR="${OPD_ROOT}/outputs/eval/q3b_q4bgrpo_nothink_full4096sftinit_plain"

RUN_TAG="${RUN_TAG:-$(date +%Y-%m-%d_%H-%M-%S)}"
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-q3b_q4bgrpo_nothink_full4096sftinit_plain_opd_r4096_b64_n1_lr1e-5_${RUN_TAG}}"
