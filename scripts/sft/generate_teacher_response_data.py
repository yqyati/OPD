...
def main() -> None:
    ...
    response_df = pd.DataFrame([completed[i] for i in range(len(df))])
    out_df = df.merge(response_df, on="__teacher_response_row_id", how="left")
    if out_df["teacher_response_text"].isna().any():
        raise RuntimeError("Merged output contains empty teacher_response_text rows.")
    out_df = out_df.drop(columns=["__teacher_response_row_id"])
