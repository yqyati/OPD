#!/usr/bin/env bash
# Formal Prefix-128 MOPD, one full epoch: 5 student GPUs + 3 frozen domain teachers.
set -euo pipefail
...
PREFIX_DATASET=datasets/mopd/q8b_mopd_math_code_instruct_teacher_prefix128_think.parquet
EXPECTED_STEP=484
CHECKPOINT_ROOT=checkpoint/mopd/q8b_pragmatic_prefix128_mopd_8gpu
ACTOR_CHECKPOINT="${CHECKPOINT_ROOT}/global_step_${EXPECTED_STEP}/actor"
MERGED_MODEL=merged_models/q8b_pragmatic_prefix128_mopd_8gpu_step${EXPECTED_STEP}
HYDRA_ARGS=()
if [ "${MOPD_CONFIG_ONLY:-0}" = "1" ]; then
  HYDRA_ARGS=(--cfg job)
fi

test -f "${PREFIX_DATASET}" || { echo "Missing Prefix-MOPD dataset: ${PREFIX_DATASET}" >&2; exit 1; }
...
if [[ "${MOPD_CONFIG_ONLY:-0}" != "1" ]] && [[ -f "${MERGED_MODEL}/config.json" ]] && compgen -G "${MERGED_MODEL}/*.safetensors" >/dev/null; then
  echo "Prefix-128 MOPD is already complete: ${MERGED_MODEL}"
  exit 0
fi
...
if [[ "${MOPD_CONFIG_ONLY:-0}" == "1" || ! -d "${ACTOR_CHECKPOINT}" ]]; then
  ...
  data.train_files="${PREFIX_DATASET}" \
  data.val_files="${PREFIX_DATASET}" \
  ...
  data.max_prompt_length=2176 +data.teacher_prefix_max_len=128 \
  data.max_response_length=4096 data.filter_overlong_prompts=True data.filter_overlong_prompts_workers=1 \
  data.truncation=error data.return_raw_chat=True \
  +data.apply_chat_template_kwargs.enable_thinking=True \
  actor_rollout_ref.model.path="${MODEL_ROOT}/Qwen3-8B-Base" actor_rollout_ref.model.use_remove_padding=True actor_rollout_ref.model.enable_gradient_checkpointing=True \
  ...
  +actor_rollout_ref.actor.teacher_prefix_sft_loss_coef=0.1 \
  actor_rollout_ref.actor.teacher_prefix_sft_loss_agg_mode=token-mean \
  actor_rollout_ref.actor.ppo_mini_batch_size=140 actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=1 \
  actor_rollout_ref.actor.use_dynamic_bsz=True actor_rollout_ref.actor.ppo_max_token_len_per_gpu=6400 \
  ...
  actor_rollout_ref.rollout.max_model_len=6400 actor_rollout_ref.rollout.max_num_batched_tokens=8192 \
  ...
  trainer.balance_batch=True trainer.val_before_train=False trainer.save_freq=484 trainer.test_freq=-1 trainer.logger='["console","tensorboard"]' \
  trainer.project_name=MOPD trainer.experiment_name=q8b_pragmatic_prefix128_mopd_8gpu \
  trainer.default_local_dir=checkpoint/mopd/q8b_pragmatic_prefix128_mopd_8gpu \
  trainer.validation_data_dir=validation_log/mopd/q8b_pragmatic_prefix128_mopd_8gpu \
  +mopd.enable=True +mopd.student_gpus=5 +mopd.advantage_clip=5 +mopd.prefix.enable=True \
...
test -d "${ACTOR_CHECKPOINT}" || {
  echo "Training exited without the required final actor checkpoint: ${ACTOR_CHECKPOINT}" >&2
  exit 1
}
test ! -e "${MERGED_MODEL}" || {
  echo "Incomplete merge target already exists; inspect it before retrying: ${MERGED_MODEL}" >&2
  exit 1
}
...
echo "Prefix-128 MOPD checkpoint and merged model are complete: ${MERGED_MODEL}"
