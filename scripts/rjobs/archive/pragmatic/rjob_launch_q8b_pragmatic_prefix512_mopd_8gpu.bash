#!/usr/bin/env bash
# Formal Prefix-512 MOPD, one full epoch: 5 student GPUs + 3 frozen domain teachers.
set -euo pipefail
...
PREFIX_DATASET=datasets/mopd/q8b_mopd_math_code_instruct_teacher_prefix512_think.parquet
EXPECTED_STEP=484
CHECKPOINT_ROOT=checkpoint/mopd/q8b_pragmatic_prefix512_mopd_8gpu
ACTOR_CHECKPOINT="${CHECKPOINT_ROOT}/global_step_${EXPECTED_STEP}/actor"
MERGED_MODEL=merged_models/q8b_pragmatic_prefix512_mopd_8gpu_step${EXPECTED_STEP}
HYDRA_ARGS=()
if [ "${MOPD_CONFIG_ONLY:-0}" = "1" ]; then
  HYDRA_ARGS=(--cfg job)
fi

test -f "${PREFIX_DATASET}" || { echo "Missing Prefix-512 MOPD dataset: ${PREFIX_DATASET}" >&2; exit 1; }
...
if [[ "${MOPD_CONFIG_ONLY:-0}" != "1" ]] && [[ -f "${MERGED_MODEL}/config.json" ]] && compgen -G "${MERGED_MODEL}/*.safetensors" >/dev/null; then
  echo "Prefix-512 MOPD is already complete: ${MERGED_MODEL}"
  exit 0
fi
...
if [[ "${MOPD_CONFIG_ONLY:-0}" == "1" || ! -d "${ACTOR_CHECKPOINT}" ]]; then
  ...
  data.max_prompt_length=2560 +data.teacher_prefix_max_len=512 \
  ...
  actor_rollout_ref.actor.use_dynamic_bsz=True actor_rollout_ref.actor.ppo_max_token_len_per_gpu=7168 \
  ...
  actor_rollout_ref.rollout.max_model_len=7168 actor_rollout_ref.rollout.max_num_batched_tokens=8192 \
  ...
  trainer.project_name=MOPD trainer.experiment_name=q8b_pragmatic_prefix512_mopd_8gpu \
  trainer.default_local_dir=checkpoint/mopd/q8b_pragmatic_prefix512_mopd_8gpu \
  trainer.validation_data_dir=validation_log/mopd/q8b_pragmatic_prefix512_mopd_8gpu \
...
echo "Prefix-512 MOPD checkpoint and merged model are complete: ${MERGED_MODEL}"
