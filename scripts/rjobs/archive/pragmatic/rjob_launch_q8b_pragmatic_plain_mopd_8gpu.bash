#!/usr/bin/env bash
# Formal 8-GPU plain MOPD, one full epoch: 5 student GPUs + 3 frozen domain teachers.
set -euo pipefail
...
EXPECTED_STEP=484
CHECKPOINT_ROOT=checkpoint/mopd/q8b_pragmatic_plain_mopd_8gpu
ACTOR_CHECKPOINT="${CHECKPOINT_ROOT}/global_step_${EXPECTED_STEP}/actor"
MERGED_MODEL=merged_models/q8b_pragmatic_plain_mopd_8gpu_step${EXPECTED_STEP}
...
if [[ -f "${MERGED_MODEL}/config.json" ]] && compgen -G "${MERGED_MODEL}/*.safetensors" >/dev/null; then
  echo "Plain MOPD is already complete: ${MERGED_MODEL}"
  exit 0
fi
...
if [[ ! -d "${ACTOR_CHECKPOINT}" ]]; then
  ...
  actor_rollout_ref.model.path="${YANGQINGYU_ROOT}/model/Qwen3-8B-Base" actor_rollout_ref.model.use_remove_padding=True actor_rollout_ref.model.enable_gradient_checkpointing=True \
  ...
  trainer.project_name=MOPD trainer.experiment_name=q8b_pragmatic_plain_mopd_8gpu \
  trainer.default_local_dir=checkpoint/mopd/q8b_pragmatic_plain_mopd_8gpu \
  trainer.validation_data_dir=validation_log/mopd/q8b_pragmatic_plain_mopd_8gpu \
...
echo "Plain MOPD checkpoint and merged model are complete: ${MERGED_MODEL}"
